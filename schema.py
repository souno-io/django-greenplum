...
class DatabaseSchemaEditor(BaseDatabaseSchemaEditor):
    sql_create_table = "CREATE TABLE %(table)s (%(definition)s) DISTRIBUTED BY (%(key)s)"
    ...
    sql_create_unique_index = "CREATE UNIQUE INDEX %(name)s ON %(table)s (%(key)s,%(columns)s)%(include)s%(condition)s"
    sql_create_index_concurrently = (
        'CREATE INDEX CONCURRENTLY %(name)s ON %(table)s%(using)s '
        '(%(columns)s)%(include)s%(extra)s%(condition)s'
    )
    sql_delete_index = "DROP INDEX IF EXISTS %(name)s"
    sql_create_unique = "ALTER TABLE %(table)s ADD CONSTRAINT %(name)s UNIQUE (%(key)s,%(columns)s)%(deferrable)s"
